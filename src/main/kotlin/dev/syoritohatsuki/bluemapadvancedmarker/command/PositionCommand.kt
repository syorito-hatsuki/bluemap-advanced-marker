        ...
        dispatcher.register(CommandManager.literal("marker")
            .then(CommandManager.literal("position").then(CommandManager.literal("add").executes {
                if (it.source.player != null) {
                    positionManager.addPosition(it.source.player!!.pos)
                } else {
                    it.source.sendError(
                ...
                if (it.source.player != null) {
                    positionManager.removePosition(it.source.player!!.pos)
                } else {
                    it.source.sendError(
                        Text.literal("Player position unavailable")
                    )
                ...
                if (context.source.player != null) {
                    ...
                    context.source.sendError(Text.literal("Player position unavailable"))
                }
                1
            })
            )
